...
const carInfo = () => {
  ...
  useEffect(() => {
    ...
    if (carInfoJson?.data?.providers) {
      const providersWithId = Object.entries(carInfoJson.data.providers).map(
        ([prv_id, providerData]) => ({
          prv_id,
          ...providerData,
        })
                ...
                <div className="flex justify-between items-center p-4 border rounded-lg shadow-sm bg-white">
                  <div className="flex flex-col space-y-1">
                    {providersArray.map((prv, id) => {
                      const quotePrvId = carInfoJson?.data.quotes[index].prv_id;
                      return quotePrvId === prv.prv_id ? (
                        <div key={prv.prv_id} className="flex items-center">
                          ...
                          <div className="flex items-center ml-2 text-orange-500">
                            {[...Array(Math.round(prv.rating))].map((_, i) => (
                              <svg
                                key={i}
                                className="h-4 w-4"
                                fill="currentColor"
                    ...
                    <div className="text-sm text-gray-600">
                      <span className="font-semibold text-lg ">
                        {carInfoJson?.data.quotes[index].new_score.toFixed(1)}
                      </span>
                    ...
                    <div className="flex items-center space-x-1 text-gray-600">
                      ...
                      <span className="text-sm capitalize">
                        {carInfoJson?.data.quotes[index].fuel_pol.replace(
                          /_/g,
                          " "
                        )}
                      </span>
                  ...
                  <div className="text-right text-gray-600 ">
                    ...
                    <a
                      ...
                      href={`https://www.skyscanner.us/${carInfoJson?.data.quotes[index].dplnk}`}
                      target="_blank"
                    >
                      Reserve
                      <svg
